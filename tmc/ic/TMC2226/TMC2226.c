...
int32_t readRegisterUART(uint16_t icID, uint8_t registerAddress)
{
	...
	if (!TMC_IS_READABLE(TMC2226.registerAccess[registerAddress]))
		return TMC2226.config->shadowRegister[registerAddress];
...
void writeRegisterUART(uint16_t icID, uint8_t registerAddress, int32_t value)
{
	...
	TMC2226.config->shadowRegister[registerAddress] = value;
	TMC2226.registerAccess[registerAddress] |= TMC_ACCESS_DIRTY;
}

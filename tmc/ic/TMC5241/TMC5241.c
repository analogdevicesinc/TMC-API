...
{
    TMC5241BusType bus = tmc5241_getBusType(icID);
...
int32_t readRegisterSPI(uint16_t icID, uint8_t address)
{
    ...
    // clear write bit
    data[0] = address & TMC5241_ADDRESS_MASK;

    // Send the read request
    tmc5241_readWriteSPI(icID, &data[0], sizeof(data));
...
void writeRegisterSPI(uint16_t icID, uint8_t address, int32_t value)
{
    ...
    data[0] = address | TMC5241_WRITE_BIT;
...
int32_t readRegisterUART(uint16_t icID, uint8_t registerAddress)
{
    ...
    registerAddress = registerAddress & TMC5241_ADDRESS_MASK;

    data[0] = 0x05;
    data[1] = tmc5241_getNodeAddress(icID); //targetAddressUart;
    data[2] = registerAddress;
    data[3] = CRC8(data, 3);

    if (!tmc5241_readWriteUART(icID, &data[0], 4, 8))
        return 0;
...
void writeRegisterUART(uint16_t icID, uint8_t registerAddress, int32_t value)
{
    ...
    data[0] = 0x05;
    data[1] = (uint8_t)tmc5241_getNodeAddress(icID); //targetAddressUart;
    data[2] = registerAddress | TMC5241_WRITE_BIT;
    ...
    tmc5241_readWriteUART(icID, &data[0], 8, 0);
}
...
void tmc5241_rotateMotor(uint16_t icID, uint8_t motor, int32_t velocity)
{
    ...
    tmc5241_writeRegister(icID, TMC5241_VMAX, (velocity < 0) ? -velocity : velocity);
    tmc5241_fieldWrite(icID, TMC5241_RAMPMODE_FIELD, (velocity >= 0) ? TMC5241_MODE_VELPOS : TMC5241_MODE_VELNEG);
}

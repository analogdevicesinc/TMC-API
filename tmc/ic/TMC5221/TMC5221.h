...
#ifndef TMC_IC_TMC5221_H_
#define TMC_IC_TMC5221_H_
...
static inline uint32_t tmc5221_fieldExtract(uint32_t data, RegisterField field)
{
...
static inline uint32_t tmc5221_fieldRead(uint16_t icID, RegisterField field)
{
    uint32_t value = tmc5221_readRegister(icID, field.address);

    return tmc5221_fieldExtract(value, field);
}
...
static inline void tmc5221_fieldWrite(uint16_t icID, RegisterField field, uint32_t value)
{
    uint32_t regValue = tmc5221_readRegister(icID, field.address);

    regValue = field_update(regValue, field, value);

    tmc5221_writeRegister(icID, field.address, regValue);
}


#endif /* TMC_IC_TMC5221_H_ */
